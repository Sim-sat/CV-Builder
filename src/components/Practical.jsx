...
function Practical({professionalEntry, listItems}) {
    ...
    const handleSubmit = (e) => {
        setShowForm(false);
        e.preventDefault();
        const newProfessional = {id:uuidv4(), companyName, title, from, end, location, description};
        setList((prev) =>{
            const updatedList = [...prev, newProfessional];
            professionalEntry(updatedList);
            return updatedList;
        } );
    ...
    return (
        <div className={clsx('text-xs bg-white  ml-5 p-5 rounded-2xl mb-5 justify-center grow-0')}  >

            <p onClick={() => setOpen(!open)} className='font-bold text-3xl cursor-pointer flex  flex-row gap-4'> 
                <MdWork className='self-center'/> 
                Experience  
                ...
                {open && !showForm && (
                    ...
                    <div className='flex content-center justify-center'>
                        <button onClick={handleButton} className='border border-grey rounded-xl p-3 text-xl text-center mt-10 hover:bg-slate-100 active:scale-95'>+ Experience</button>
                    </div>
                    </>
                )}
                    {showForm && open && (
                        <form>
                            <label htmlFor="name" className='font-bold text-base'>Company Name</label><br></br>
                            <input onChange={handleNameChange}
                            type="text"
                            id="name" 
                            placeholder='First Company Name' 
                            className="m-2 w-full rounded-xl bg-slate-100 p-2 text-base text-base/1" ></input><br></br>
                            <label htmlFor="tile" className='font-bold  text-base'>Title</label><br></br>
                            ...
                            <input onChange={handleEndChange}
                                ...
                                type="date"></input><br></br>
                            <label htmlFor="location" className='font-bold  text-base'>Location</label><br></br>
                            <input onChange={handleLocationChange}
                                id="location"
                                placeholder='City, Country'
                                className="m-2 rounded-xl w-full bg-slate-100  p-2 text-base" 
                                type="text"></input><br></br>
                            <label htmlFor="description" className='font-bold  text-base'>Description</label><br></br>
